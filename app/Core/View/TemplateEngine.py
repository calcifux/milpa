"""Motor de templates Jinja2 del framework: vistas web + correos (= Blade del legacy).

Vive en `app/Core/View` (capa de templating reutilizable): tanto el `Mailer`
(`Core/Mail`) como el helper `view()` (`Core/View`) lo consumen. No es exclusivo
de correo.

Mapeo verificado contra `~/mse_grouped_fork_dev/resources/views/emails/`:
...
from app.Core.Translate import t as default_translate

# Raíz del proyecto (…/app/Core/View/TemplateEngine.py -> parents[3]).
_PROJECT_ROOT = Path(__file__).resolve().parents[3]
# Raíz ÚNICA de vistas compartidas: app/Resources/Views. Todo se referencia
# relativo a aquí (convención Jinja): "index.html.j2", "Emails/Trans/master.html.j2".
_WEB_VIEWS_DIR = _PROJECT_ROOT / "app" / "Resources" / "Views"
...
def _module_views_dirs() -> dict[str, Path]:
    ...
    modules_root = _PROJECT_ROOT / "app" / "Modules"
...
def _build_loader(templates_dir: Path | None) -> BaseLoader:
    ...
    a aquí: "index.html.j2", "Emails/Trans/master.html.j2"). Convención 100% Jinja:
...
class TemplateEngine:
    ...
    (`Modules/<X>/Resources/Views`, namespaced "<x>/...") + `app/Resources/Views`
    (raíz default: layouts web) + `app/Resources/Views/Emails` (fallback de correos).
    ...
    def __init__(
        ...
        # `asset()` (= asset() de Laravel): construye la URL de un estatico servido
        # bajo /static, para CSS/JS en vistas web sin hardcodear la ruta. Compartido:
