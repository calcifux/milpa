...
# un request (cron, CLI, worker) es None → `t()` cae al app_fallback_locale.
...
def current_locale() -> str:
    """Locale del request actual (lo fijó la dependency global desde Accept-Language).

    Evita el boilerplate de `Header(...) + resolve_accept_language(...)` en cada
    endpoint: en un request ya está resuelto en el contextvar. Úsalo para CAPTURAR
    el locale y pasarlo a un Mailable que se ENCOLA (el worker no ve el contextvar).
    Fuera de un request (cron/CLI/worker) cae al `app_fallback_locale`.
    """
    return _request_locale.get() or settings.app_fallback_locale


def resolve_accept_language(header: str) -> str:
    """Resuelve el locale desde el header Accept-Language: toma el idioma de mayor
    `q`, reducido a su subtag primario ('es-MX' -> 'es'). Vacío/no parseable → cae
    al app_fallback_locale. (i18nice ya hace fallback si el catálogo no existe.)"""
    best_quality = -1.0
    best_locale = settings.app_fallback_locale
...
_LANG_DIR = _PROJECT_ROOT / "app" / "Resources" / "Lang"
...
def t(
    ...
    # Prioridad: locale explícito > locale del request (Accept-Language) > fallback config.
