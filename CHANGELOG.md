...
- **`milpa.lazy`** — import perezoso opt-in para libs pesadas/opcionales. El auto-discovery
  ...
  efímero). `from milpa.lazy import openpyxl` difiere la carga hasta el primer uso (vía
...
- **`jornal scan` (`milpa.scan`)** — análisis automático y extensible de la app. Construye un
  modelo (grafo de imports al top-level + estado eager/lazy, vía el discovery que ya existe) y
  corre **capacidades** por concern: `lazy` (libs pesadas eager → sugiere `milpa.lazy`, acredita
