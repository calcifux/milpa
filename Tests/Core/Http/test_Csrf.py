...
def test_unsafe_without_token_is_forbidden() -> None:
...
def test_unsafe_with_matching_token_passes() -> None:
    client = TestClient(_app())
    client.get("/safe")  # siembra la cookie CSRF (TestClient la guarda)
...
def test_unsafe_with_mismatched_token_is_forbidden() -> None:
    client = TestClient(_app())
    client.get("/safe")
...
def test_bearer_requests_are_exempt() -> None:
    response = TestClient(_app()).post("/unsafe", headers={"Authorization": "Bearer abc"})
    assert response.status_code == 200
